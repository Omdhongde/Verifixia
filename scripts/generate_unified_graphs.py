...
def generate_graphs():
    print("Loading training history JSON files...")
    ...
    print("Generating high-resolution 3-model comparative graphs...")
    ...
    # ==================== ROW 2: DEEPERFORENSICS VIDEO DETECTOR (SE-ResNet) ====================
    ...
    axs[1, 1].set_title("Video Model (DeeperForensics): Classification Accuracy Curves", fontsize=13, fontweight="bold", pad=8)
    ...
    # ==================== ROW 3: MULTI-CLASS DETECTOR (SE-Attention 3-Class) ====================
    ...
    axs[2, 0].set_title("Multi-Class Model: Cross-Entropy Loss Progression (80 Epochs)", fontsize=13, fontweight="bold", pad=8)
    axs[2, 0].set_xlabel("Epochs", fontsize=10)
    axs[2, 0].set_ylabel("Loss", fontsize=10)
    axs[2, 0].legend(frameon=True, facecolor="white", framealpha=0.9, fontsize=9)
    ...
    axs[2, 1].set_title("Multi-Class Model: Classification Accuracy Curves", fontsize=13, fontweight="bold", pad=8)
    axs[2, 1].set_xlabel("Epochs", fontsize=10)
    axs[2, 1].set_ylabel("Accuracy (%)", fontsize=10)
    axs[2, 1].legend(frameon=True, facecolor="white", framealpha=0.9, fontsize=9)
    axs[2, 1].grid(True, alpha=0.4)
    
    plt.suptitle("Verifixia Deepfake Detection Models - Training Progression Curves", fontsize=18, fontweight="bold", y=0.97)
    plt.tight_layout(rect=[0, 0.02, 1, 0.95])
    
    output_png = models_dir / "unified_training_history.png"
    plt.savefig(output_png, dpi=300, bbox_inches="tight")
    plt.close()
    
    print(f"[OK] High-resolution graphs saved successfully to {output_png.resolve()}")
